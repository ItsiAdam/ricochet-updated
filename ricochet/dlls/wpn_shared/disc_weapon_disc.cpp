...
void CDisc::Spawn(void)
{
	...
	if (pPlayer->HasPowerup(POW_VISUALIZE_REBOUNDS))
	{
		Vector vecFireDir = g_vecZero;
		Vector vecSrc = pPlayer->pev->origin + (pPlayer->pev->view_ofs * 0.25);
		vecFireDir[1] = pPlayer->pev->v_angle[1];

		for (int i = 0; i < 5; i++)
		{
			TraceResult tr;
			UTIL_MakeVectors(vecFireDir);
			UTIL_TraceLine(vecSrc, (vecSrc + gpGlobals->v_forward * 2048), dont_ignore_monsters, ENT(pev), &tr);
	...
	if (m_iPowerupFlags & POW_VISUALIZE_REBOUNDS)
...
void CDiscWeapon::WeaponIdle(void)
{
	...
	if (m_pPlayer->HasPowerup(POW_VISUALIZE_REBOUNDS))
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			// if texture is sky stop drawing
