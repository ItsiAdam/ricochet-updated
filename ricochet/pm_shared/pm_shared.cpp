...
void PM_Duck(void)
{
	...
	// this shouldn't be here but i can't be arsed figuring out where it should be
	g_iEnableCrouch = g_engfuncs.pfnCVarGetFloat("rc_enable_crouch");
	// Discwar: Prevent ducking
	if (g_iEnableCrouch == 0)
		return;
...
void PM_Jump(void)
{
	...
	g_iEnableJump = g_engfuncs.pfnCVarGetFloat("rc_enable_jump");
	if (g_iEnableJump == 0)
		return;
